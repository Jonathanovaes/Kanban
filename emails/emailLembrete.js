function email(response){
    ...
    (async()=>{
    let link = `https://almeidaenogueira.novajus.com.br/servicos/compromissos/editcompromisso/${response.id}?parentId=3319&tipoContexto=2&returnUrl=%2Fservicos%2Fservicos%2FDetailsCompromissosTarefas%2F3319%3Fajaxnavigation%3Dtrue%26renderOnlySection%3DTrue`
    ...
    if(nome=="ANA GABRIELA DE GOES MONTAI"||nome=="MARCUS VINICIUS FERREIRA DE RABELO ARRUDA"||nome=="GABRIELA DE ALMEIDA DOMINGUES QUINTEIRO"||nome=="MARCELA BITTENCOURT DA SILVEIRA"||nome=="ARIADNE FABIANE VELOSA CAVALCANTI"){
    ...
    const mailOptions = {
        ...
        to: [`legalbot@almeidaenogueira.com.br`,`legalbot@almeidaenogueira.com.br`], // receiver (use array of string for a list)
        subject: `Você tem um compromisso pendente para amanhã!`, // Subject line
        html: `<p>Prezados,<br><br>O compromisso ${response.id}, tem conclusão prevista para amanhã.<br><br>Segue a descrição do compromisso: ${response.description}<br><br>Aqui está o link de atalho para o compromisso pendente: ${link}<br><br>Atenciosamente,<br><br>api do Legal Bot</p>`// plain text body
        };
